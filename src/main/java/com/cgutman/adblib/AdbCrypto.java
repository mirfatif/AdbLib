...
/**
 * This class encapsulates the ADB cryptography functions and provides an interface for the storage
...
public class AdbCrypto {
  ...
  public static AdbCrypto loadAdbKeyPair(AdbBase64 base64, File privateKey, File publicKey)
    ...
    if (privIn.read(privKeyBytes) == -1) throw new IOException("Empty private key file");
    if (pubIn.read(pubKeyBytes) == -1) throw new IOException("Empty public key file");
    ...
    crypto.keyPair =
        new KeyPair(
            keyFactory.generatePublic(publicKeySpec), keyFactory.generatePrivate(privateKeySpec));
