...
public class AdbProtocol {
  ...
  /**
   * CNXN is the connect message. No messages (except AUTH) are valid before this message is
   * received.
   */
  ...
  /**
   * AUTH is the authentication message. It is part of the RSA public key authentication added in
   * Android 4.2.2.
   */
  ...
  /**
   * WRTE is the write stream message. It is sent with a payload that is the data to write to the
   * stream.
   */
  ...
  public static byte[] generateMessage(int cmd, int arg0, int arg1, byte[] payload) {
    ...
    if (payload != null) {
      message =
          ByteBuffer.allocate(ADB_HEADER_LENGTH + payload.length).order(ByteOrder.LITTLE_ENDIAN);
    } else {
